...
const RELEASE_ID='0.20-oereb';
const RELEASE_NOTES={
  title:'Beta 0.20 – ÖREB-Grundstücksprüfung',
  date:'21.08.2026',
  notes:[
    'Der Standortcheck versucht neu, das Grundstück über den kantonalen ÖREB-Webservice eindeutig per EGRID zuzuordnen.',
    'Unterstützt werden die Pilotkantone Aargau, Basel-Landschaft, Basel-Stadt und Solothurn.',
    'Bei erfolgreicher Zuordnung stehen direkte Links zur amtlichen ÖREB-Information und zum PDF-Auszug bereit.',
    'Eine gefundene EGRID erhöht die Datenqualität der Erstprüfung.',
    'Die bestehende Speicher-, Pipeline-, Standort- und Szenario-Logik bleibt erhalten.'
  ]
};
...
function setVisibleVersion(){
  ...
  if(!b){const actions=document.querySelector('.top-actions');if(actions){b=document.createElement('button');b.type='button';b.id='versionBadge';b.className='version-pill';actions.prepend(b)}}
...
function fillReleaseNotes(){
  ...
  t.textContent='Was ist neu in '+RELEASE_NOTES.title+'?';d.textContent=RELEASE_NOTES.date;
  n.innerHTML=RELEASE_NOTES.notes.map(x=>`<div class="patch-item"><span>✓</span><p>${escU(x)}</p></div>`).join('');return true;
}
...
function showReleaseNotes(force=false){
  ...
  const d=document.getElementById('patchDialog');if(d&&!d.open)try{d.showModal()}catch{}
}

function bindReleaseClose(){
  const b=document.getElementById('patchCloseBtn');if(!b||b.dataset.releaseBound==='1')return;
  b.dataset.releaseBound='1';b.addEventListener('click',()=>{localStorage.setItem('structaScoutReleaseSeen',RELEASE_ID);const d=document.getElementById('patchDialog');if(d?.open)d.close()});
}

function installPullToRefresh(){
  if(document.getElementById('pullRefreshIndicator'))return;
  const i=document.createElement('div');i.id='pullRefreshIndicator';i.className='pull-refresh';i.textContent='Zum Aktualisieren nach unten ziehen';document.body.appendChild(i);
...
function renderVerifiedResults(data){
  ...
  const items=verifiedFilter(data.items||[]);window.__structaVerifiedItems=items;
  ...
  status.hidden=false;status.innerHTML=`<strong>${items.length} passende Marktangebote.</strong> „Jetzt prüfen“ übernimmt das Objekt in die Analyse.`;
}

async function runVerifiedMarketSearch(e){
  if(e){e.preventDefault();e.stopImmediatePropagation()}
  const status=document.getElementById('searchStatus'),target=document.getElementById('liveResults');if(status){status.hidden=false;status.innerHTML='<strong>Markt wird geprüft …</strong>'}if(target)target.innerHTML='';
  try{const r=await fetch('./market-data.json?t='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error();renderVerifiedResults(await r.json())}
  catch{if(status)status.innerHTML='<strong>Marktstand konnte gerade nicht geladen werden.</strong> Die direkten Suchbereiche stehen weiterhin zur Verfügung.'}
  finally{if(typeof buildFallbacks==='function')buildFallbacks()}
}

function installMarketHandlers(){
  const target=document.getElementById('liveResults');
  if(target&&target.dataset.checkBound!=='1'){target.dataset.checkBound='1';target.addEventListener('click',e=>{const b=e.target.closest('.market-check-btn');if(!b)return;const x=(window.__structaVerifiedItems||[])[Number(b.dataset.marketIndex)];if(x&&typeof window.openMarketOpportunity==='function')window.openMarketOpportunity(x)})}
  const button=document.getElementById('startSearchBtn');if(button&&button.dataset.verifiedBound!=='1'){button.dataset.verifiedBound='1';button.addEventListener('click',runVerifiedMarketSearch,true)}
  const query=document.getElementById('searchKeywords');if(query&&query.dataset.verifiedBound!=='1'){query.dataset.verifiedBound='1';query.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();e.stopImmediatePropagation();runVerifiedMarketSearch()}},true)}
}

function scrollToPipeline(sortValue=null){
  const sort=document.getElementById('sortFilter');if(sortValue&&sort){sort.value=sortValue;sort.dispatchEvent(new Event('change',{bubbles:true}))}
  const target=document.querySelector('.opportunities');if(target)setTimeout(()=>target.scrollIntoView({behavior:'smooth',block:'start'}),30);
}
